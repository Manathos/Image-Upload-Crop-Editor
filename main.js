const canvasEl = document.getElementById('c');
    ...
    function calculateDPI(imageWidth, imageHeight) {
      // Calculate DPI based on original image size vs physical pad dimensions
      let physicalWidth, physicalHeight;
      
      // Get physical dimensions based on current preset
      switch (currentPreset) {
        case 'extended':
          physicalWidth = 33; // 33 inches
          physicalHeight = 11; // 11 inches
          break;
        case 'immersive':
          physicalWidth = 33.5; // 33.5 inches
          physicalHeight = 13.8; // 13.8 inches
          break;
        case 'laptop':
        case 'playmat':
          physicalWidth = 23.6; // 23.6 inches
          physicalHeight = 13.8; // 13.8 inches
          break;
        case 'fullsize':
          physicalWidth = 37.4; // 37.4 inches
          physicalHeight = 15.7; // 15.7 inches
          break;
        case 'supersize':
          physicalWidth = 39.1; // 39.1 inches
          physicalHeight = 19.6; // 19.6 inches
          break;
        default:
          physicalWidth = 33; // Default to extended
          physicalHeight = 11;
      }
      
      // Calculate DPI as pixels per inch
      const dpiX = imageWidth / physicalWidth;
      const dpiY = imageHeight / physicalHeight;
    ...
    function updateQualityNotification(effectiveDpi) {
      const notificationBadge = document.querySelector('.quality-notification-badge');
      if (!notificationBadge) return;
      
      // Show notification for low quality images (below 72 DPI)
      if (effectiveDpi < 72) {
    ...
    function undoAutoEnlargement(imageId) {
      // Find the image object by ID
      const imageObject = deskPadModalCanvas.getObjects().find(obj => obj._imageId === imageId);
      if (!imageObject || !imageObject._wasAutoEnlarged) return;
      
      // Reset to original scale (1.0 = no scaling)
      imageObject.set({
        scaleX: 1,
        scaleY: 1,
        _userHasResized: false, // Reset user resize flag
        _wasAutoEnlarged: false // Mark as no longer auto-enlarged
      });
      ...
      if (activeLayerId) {
        ...
        if (activeLayer && activeLayer.object === imageObject) {
    ...
    window.undoAutoEnlargement = undoAutoEnlargement;
      ...
      function actuallyApplyRotation() {
        ...
        if (currentOrigFabricImg._element && currentOrigFabricImg._element.complete) {
          ...
          fabric.Image.fromURL(tempCanvas.toDataURL(), newImg => {
            ...
            // Preserve proxy system properties for rotated/flipped images
            newImg.imageId = currentOrigFabricImg.imageId;
            newImg.name = currentOrigFabricImg.name;
            newImg._originalDataUrl = currentOrigFabricImg._originalDataUrl;
            
            // Preserve original dimensions for DPI calculation
            newImg._originalWidth = currentOrigFabricImg._originalWidth;
            newImg._originalHeight = currentOrigFabricImg._originalHeight;
            
            // Preserve any other proxy system properties
            if (currentOrigFabricImg._proxyDataUrl) {
              newImg._proxyDataUrl = currentOrigFabricImg._proxyDataUrl;
            }
            if (currentOrigFabricImg._originalFile) {
              newImg._originalFile = currentOrigFabricImg._originalFile;
            }
            
            // Mark as user-modified (rotated/flipped) to hide auto-enlargement warning
            newImg._userHasResized = true;
            newImg._imageId = currentOrigFabricImg._imageId; // Preserve image ID
    ...
    function enterCropMode() {
      ...
      // Ensure crop box renders above canvas
      try { cropModalImgContainer.appendChild(cropModalCropbox); } catch(_) {}
      ...
      // Ensure crop box is properly initialized after canvas paints
      requestAnimationFrame(() => requestAnimationFrame(() => {
        let cropRect = cropModalCropbox.getBoundingClientRect();
        if (cropRect.width === 0 || cropRect.height === 0) {
          cropModalCropbox.style.transform = 'none';
        }
        // Fallback if still zero size
        cropRect = cropModalCropbox.getBoundingClientRect();
        if (cropRect.width === 0 || cropRect.height === 0) {
          const w = cropModalCanvas.offsetWidth;
          const h = cropModalCanvas.offsetHeight;
          const cw = Math.max(50, Math.round(w * 0.6));
          const ch = Math.max(50, Math.round(h * 0.6));
          cropModalCropbox.style.left = Math.round((w - cw) / 2) + 'px';
          cropModalCropbox.style.top = Math.round((h - ch) / 2) + 'px';
          cropModalCropbox.style.width = cw + 'px';
          cropModalCropbox.style.height = ch + 'px';
          cropModalCropbox.style.display = 'block';
        }
      }));
    }
      ...
      return new Promise((resolve, reject) => {
        fabric.Image.fromURL(tempCanvas.toDataURL(), newImg => {
          ...
          try {
            ...
            // Preserve proxy system properties for cropped images
            newImg.imageId = origFabricImg.imageId;
            newImg.name = origFabricImg.name;
            newImg._originalDataUrl = origFabricImg._originalDataUrl;
            
            // Preserve original dimensions for DPI calculation
            newImg._originalWidth = origFabricImg._originalWidth;
            newImg._originalHeight = origFabricImg._originalHeight;
            
            // Preserve any other proxy system properties
            if (origFabricImg._proxyDataUrl) {
              newImg._proxyDataUrl = origFabricImg._proxyDataUrl;
            }
            if (origFabricImg._originalFile) {
              newImg._originalFile = origFabricImg._originalFile;
            }
            
            // Mark as user-modified (cropped) to hide auto-enlargement warning
            newImg._userHasResized = true;
            newImg._imageId = origFabricImg._imageId; // Preserve image ID
    ...
    function addImageToCanvas(img, index = 0) {
      ...
      if (index === 0) {
        ...
        const wasAutoEnlarged = scale > 1;
        
        // Generate unique ID for this image
        const imageId = 'img_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
        
        console.log('Image upload tracking:', {
        ...
        img.set({
          ...
          _originalScale: scale, // Store original scale for undo
          _userHasResized: false, // Track if user has manually resized
          _imageId: imageId // Unique identifier
        });
    ...
    let currentDPI = 72; // Default to 72 DPI for optimal performance
    let currentCanvasWidth = 2088; // Default 72 DPI width (77:29 aspect ratio)
    let currentCanvasHeight = 787; // Default 72 DPI height (77:29 aspect ratio)
    ...
    const aspectPresets = {
      extended:    { w: 4725, h: 1785, aspect: 77/29, name: 'EXTENDED', proof: 'proof_extended.png' },    // 33" x 11" @150dpi
      immersive:   { w: 5310, h: 2220, aspect: 43/18, name: 'IMMERSIVE', proof: 'proof_immersive.png' },   // 33.5" x 13.8" @150dpi
      laptop:      { w: 3815, h: 2250, aspect: 61/36, name: 'LAPTOP', proof: 'proof_laptop.png' },      // 23.6" x 13.8" @150dpi
      playmat:     { w: 3815, h: 2250, aspect: 61/36, name: 'PLAY MAT', proof: 'proof_laptop.png' },    // 23.6" x 13.8" @150dpi (2mm thickness)
      fullsize:    { w: 5400, h: 2306, aspect: 96/41, name: 'FULLSIZE', proof: 'proof_fullsize.png' },    // 37.4" x 15.7" @150dpi
      supersize:   { w: 7200, h: 3636, aspect: 101/51, name: 'SUPERSIZE', proof: 'proof_supersize.png' }   // 39.1" x 19.6" @150dpi
    };
    ...
    // Expose DPI settings to proxy system
    window.currentDPI = currentDPI;
    window.EXPORT_DPI = EXPORT_DPI;
    ...
    function downloadCanvas() {
      console.log('downloadCanvas called');
      ...
      // Hide selection handles for clean download
      const prevActive = deskPadModalCanvas.getActiveObject();
      deskPadModalCanvas.discardActiveObject();
      deskPadModalCanvas.renderAll();
      ...
      link.download = `deskpad_design_${Date.now()}.png`;
      link.href = deskPadModalCanvas.toDataURL({
        format: 'png',
        quality: 1.0,
        multiplier: 1.0
      });
    ...
    function applyAspectPreset(presetKey) {
      ...
      if (totalPixels > 10000000) { // 10 million pixels
        console.warn(`🚨 Large canvas detected: ${canvasWidth} — ${canvasHeight} = ${(totalPixels/1000000).toFixed(1)}M pixels`);
    ...
    function updateCanvasResolution(dpi, width, height) {
      ...
      // Update window variables for proxy system
      window.currentDPI = currentDPI;
      window.EXPORT_DPI = EXPORT_DPI;
    ...
    function updateModalDPIDisplay(imageWidth, imageHeight, canvasWidth, canvasHeight, scale, originalWidth = null, originalHeight = null) {
      // Use original dimensions if available (for proxy images), otherwise use provided dimensions
      const effectiveWidth = originalWidth || imageWidth;
      const effectiveHeight = originalHeight || imageHeight;
      
      const dpi = calculateDPI(effectiveWidth, effectiveHeight);
      const scaledWidth = Math.round(imageWidth * scale);
      const scaledHeight = Math.round(imageHeight * scale);
      
      // Get physical dimensions for display
      let physicalWidth, physicalHeight;
      switch (currentPreset) {
        case 'extended':
          physicalWidth = 33; physicalHeight = 11; break;
        case 'immersive':
          physicalWidth = 33.5; physicalHeight = 13.8; break;
        case 'laptop':
        case 'playmat':
          physicalWidth = 23.6; physicalHeight = 13.8; break;
        case 'fullsize':
          physicalWidth = 37.4; physicalHeight = 15.7; break;
        case 'supersize':
          physicalWidth = 39.1; physicalHeight = 19.6; break;
        default:
          physicalWidth = 33; physicalHeight = 11;
      }
      
      modalDpiDisplay.innerHTML = `
        <strong>Modal Image DPI Info:</strong><br>
        Image: ${effectiveWidth} — ${effectiveHeight}px<br>
        Physical: ${physicalWidth}" — ${physicalHeight}"<br>
    ...
    function initResizableDeskPadModalCanvas() {
      ...
      fixTextBaselineErrors();
          ...
          deskPadModalCanvas.on('object:scaling', function(opt) {
            // Track if user has manually resized the image
            if (opt.target && opt.target.type === 'image') {
              opt.target._userHasResized = true;
            }
          ...
          deskPadModalCanvas.on('object:modified', function(opt) {
            // Hide center guide lines when object stops moving
            showCenterGuideLines(false);
    ...
    function showTextEditorModal(obj) {
      ...
      if (!modal) {
              ...
              <div id="strokeControls" style="display: none;">
                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 0.5rem;">
                  ...
                  <div>
                    ...
                    <div style="display: flex; gap: 0.25rem;">
                      <button id="strokeOuter" class="stroke-position-btn" data-position="outer" style="flex: 1; padding: 3px 6px; border-radius: 3px; cursor: pointer; font-size: 10px;">Outer</button>
                      <button id="strokeCenter" class="stroke-position-btn active" data-position="center" style="flex: 1; padding: 3px 6px; border-radius: 3px; cursor: pointer; font-size: 10px;">Center</button>
                    </div>
                  </div>
                </div>
              </div>
        ...
        // Set initial button styles based on theme
        const strokePositionBtns = modal.querySelectorAll('.stroke-position-btn');
        const isLightTheme = document.documentElement.getAttribute('data-theme') === 'light';
        strokePositionBtns.forEach(btn => {
          const isActive = btn.classList.contains('active');
          if (isActive) {
            btn.style.setProperty('background', '#f4a012', 'important');
            btn.style.setProperty('color', '#fff', 'important');
            btn.style.setProperty('border-color', '#f4a012', 'important');
            btn.style.setProperty('border', '1px solid #f4a012', 'important');
          } else {
            btn.style.setProperty('background', isLightTheme ? '#e0e0e0' : '#444', 'important');
            btn.style.setProperty('color', isLightTheme ? '#333' : '#ccc', 'important');
            btn.style.setProperty('border-color', isLightTheme ? '#ccc' : '#555', 'important');
            btn.style.setProperty('border', isLightTheme ? '1px solid #ccc' : '1px solid #555', 'important');
          }
        });
      ...
      if (obj) {
        ...
        // Get the current font size from the object
        // Calculate the effective visual size (fontSize × scale)
        const baseFontSize = obj.fontSize || 45;
        const scaleX = obj.scaleX || 1;
        const scaleY = obj.scaleY || 1;
        const effectiveScale = Math.max(scaleX, scaleY);
        const currentFontSize = Math.round(baseFontSize * effectiveScale);
        ...
        // Use remembered stroke position, but load from object if available
        strokePosition = obj.strokePosition || lastModalState.strokePosition;
        ...
        // Also update button states for other controls
        updateBtnStates();
      } else {
        textInput.value = '';
        fontSel.value = 'Arial';
      ...
      modal.querySelector('#modalApply').onclick = function() {
        ...
        if (editingTextObj) {
          ...
          editingTextObj.set({ 
            ...
            text: finalText,
            fill: finalFill,
            scaleX: 1, // Reset scale to 1 when applying new font size
            scaleY: 1, // Reset scale to 1 when applying new font size
            selectable: true,
            evented: true
          });
      ...
      if (recentImages && recentImages.length > 0) {
        recentImages.forEach((imageData, index) => {
          ...
          imageItem.addEventListener('click', function() {
            addImageToModalCanvasWithMode(imageData.url, imageData.name, 0);
            hideRecentImagesPopup();
          });
    ...
    function applySimpleSnapping(obj) {
      ...
      // Check if object is near center (for guide lines)
      ...
      if (distanceFromCenterX < centerGuideThreshold || distanceFromCenterY < centerGuideThreshold) {
      ...
      // Show/hide center guide lines
      showCenterGuideLines(showCenterGuides);
    }
    
    // Show or hide center guide lines
    function showCenterGuideLines(show) {
      const verticalGuide = document.getElementById('center-guide-vertical');
      const horizontalGuide = document.getElementById('center-guide-horizontal');
      
      if (verticalGuide && horizontalGuide) {
        const opacity = show ? 0.6 : 0;
        verticalGuide.style.opacity = opacity;
        horizontalGuide.style.opacity = opacity;
      }
    }
    ...
    function setupCanvasZoomAndPan(canvas) {
      ...
      if (canvas === deskPadModalCanvas) {
        ...
        const canvasContainer = canvas.wrapperEl || canvas.upperCanvasEl?.parentElement;
        if (canvasContainer) {
    ...
    deskPadButton.addEventListener('click', function() {
      ...
      setTimeout(() => {
        ...
        // Setup mobile interface if on mobile
        if (window.setupMobileInterface) {
          // Ensure mobile interface is ready before setting it up
          if (window.ensureMobileInterfaceReady) {
            window.ensureMobileInterfaceReady();
          }
          window.setupMobileInterface();
        }
    ...
    let deskPadDownloadBtn = document.getElementById('desk-pad-download-btn');
    
    deskPadDownloadBtn.addEventListener('click', function() {
      console.log('Download button clicked!');
      downloadCanvasWithMode();
    });
    ...
    function updateRecentImages() {
      ...
      sessionImages.slice().reverse().forEach((imageData, index) => {
        ...
        imageItem.addEventListener('click', () => {
          ...
          addImageToModalCanvasWithMode(imageData);
          closeAddImageModal();
    ...
    function addImageToModalCanvasInternal(img, index = 0) {
      ...
      // Check if this is a proxy image with original dimensions
      const originalWidth = img._originalWidth || null;
      const originalHeight = img._originalHeight || null;
      updateModalDPIDisplay(img.width, img.height, modalCanvasWidth, modalCanvasHeight, scale, originalWidth, originalHeight);
    ...
    function handleModalFileUpload(files) {
      ...
      if (loadingOverlay) {
        ...
        const loadingOverlayRetry = document.getElementById('loading-overlay');
        if (loadingOverlayRetry) {
          loadingOverlayRetry.classList.add('show');
        }
      }
      ...
      window.modalImageProcessingComplete = () => {
        ...
        // Update loading text
        loadingText.textContent = `Uploading image ${processedCount} of ${imageFiles.length}...`;
      ...
      imageFiles.forEach((file, index) => {
        ...
        reader.onload = (e) => {
                      addImageToModalCanvasWithMode(e.target.result, file.name, index);
        };
    ...
    function setupModalEventListeners() {
      ...
      // Image Properties Toggle Functionality
      const imagePropertiesHeader = document.getElementById('image-properties-header');
      if (imagePropertiesHeader) {
        imagePropertiesHeader.addEventListener('click', function() {
          const content = document.getElementById('image-properties-content');
          const toggle = document.getElementById('image-properties-toggle');
    ...
    function setupThemeToggle() {
      ...
      const logoImg = document.querySelector('#desk-pad-modal-toolbar img[alt="Paddash Logo"]');
      if (logoImg && savedTheme === 'light') {
        logoImg.src = 'Paddash.com Logo black text.png';
      }
      ...
      themeToggleBtn.addEventListener('click', () => {
        ...
        styleSheets.forEach(sheet => {
          ...
          sheet.href = href + separator + 'v=' + Date.now();
        });
      ...
      function updateThemeToggleDisplay(theme) {
        if (theme === 'light') {
          ...
          // Update logo to black text version for light theme
          const logoImg = document.querySelector('#desk-pad-modal-toolbar img[alt="Paddash Logo"]');
          if (logoImg) {
            logoImg.src = 'Paddash.com Logo black text.png';
          }
          ...
          // Update logo to original version for dark theme
          const logoImg = document.querySelector('#desk-pad-modal-toolbar img[alt="Paddash Logo"]');
          if (logoImg) {
            logoImg.src = 'Paddash Logo.png';
          }
        }
      }
    }
    ...
    deskPadModalCanvasContainer.addEventListener('drop', e => {
      ...
      const loadingText = document.getElementById('loading-text');
              loadingText.textContent = `Uploading image 1 of ${imageFiles.length}...`;
      ...
      imageFiles.forEach((file, index) => {
        const reader = new FileReader();
        reader.onload = ev => {
                      addImageToModalCanvasWithMode(ev.target.result, file.name, index);
        };
        reader.readAsDataURL(file);
      });
    });
    ...
    function resetModalCanvasToDefault() {
      ...
      const currentDpiDisplay = document.getElementById('current-dpi-display');
      if (currentDpiDisplay) {
        currentDpiDisplay.textContent = `72 DPI • 2088 × 787px`;
      }
      
              // Reset handle sizes to default (72 DPI)
              updateHandleSizes(72);
    ...
    function updateLayersList() {
      ...
      // Update arrow button states after updating the layers list
      updateLayerArrowButtonStates();
    }
    ...
    function updatePropertiesPanel(layer) {
      ...
      if (!layer || !layer.object) {
        ...
        if (selectedObjects.length > 1) {
          ...
          `;
          // Hide quality notification when multiple objects are selected
          updateQualityNotification(999); // Pass high DPI to hide notification
        } else {
          objectPreviewContainer.style.display = 'none';
          propertiesContent.innerHTML = `
            <div style="color: #ccc; font-size: 12px; font-style: italic;">
          ...
          `;
          // Hide quality notification when no object is selected
          updateQualityNotification(999); // Pass high DPI to hide notification
        }
        
        // Hide image properties container when no object is selected
        const imagePropertiesContainer = document.getElementById('image-properties-container');
        if (imagePropertiesContainer) {
          imagePropertiesContainer.style.display = 'none';
        }
      ...
      // Get the object early so we can use it in image properties
      const obj = layer.object;
      
      // Handle image properties container
      const imagePropertiesContainer = document.getElementById('image-properties-container');
      const imageDisplaySize = document.getElementById('image-display-size');
      const imageOriginalSize = document.getElementById('image-original-size');
      ...
      if (imagePropertiesContainer && obj.type === 'image') {
        // Show image properties container for images
        imagePropertiesContainer.style.display = 'block';
        ...
        // Update the image properties spans
        if (imageDisplaySize) imageDisplaySize.textContent = `${displayWidth} × ${displayHeight}px`;
        if (imageOriginalSize) imageOriginalSize.textContent = `${originalWidth} × ${originalHeight}px`;
        if (imageScale) imageScale.textContent = `${scaleX}% × ${scaleY}%`;
        if (imageRotation) imageRotation.textContent = `${rotation}°`;
        if (imagePosition) imagePosition.textContent = `X: ${x}, Y: ${y}`;
      } else if (imagePropertiesContainer) {
        // Hide image properties container for non-image objects
        imagePropertiesContainer.style.display = 'none';
      }
      ...
      if (obj.type === 'image') {
        ...
        // Calculate DPI for the image using original dimensions vs physical pad size
        // DPI should be based on original image quality, not current scale
        const effectiveWidth = obj._originalWidth || originalWidth;
        const effectiveHeight = obj._originalHeight || originalHeight;
        
        const dpiInfo = calculateDPI(effectiveWidth, effectiveHeight);
        const dpiXQuality = getDPIQualityInfo(dpiInfo.dpiX);
        const dpiYQuality = getDPIQualityInfo(dpiInfo.dpiY);
        
        // Calculate Quality Impact based on current scale
        const currentScaleX = obj.scaleX || 1;
        const currentScaleY = obj.scaleY || 1;
        const maxScale = Math.max(currentScaleX, currentScaleY);
        
        // Calculate effective DPI (original DPI / current scale)
        const effectiveDpiX = Math.round(dpiInfo.dpiX / maxScale);
        const effectiveDpiY = Math.round(dpiInfo.dpiY / maxScale);
        
        // Calculate quality loss/gain percentage
        const qualityLossX = Math.round(((dpiInfo.dpiX - effectiveDpiX) / dpiInfo.dpiX) * 100);
        const qualityLossY = Math.round(((dpiInfo.dpiY - effectiveDpiY) / dpiInfo.dpiY) * 100);
        
        // Get quality info for effective DPI using the same color system
        const effectiveDpiQuality = getDPIQualityInfo(effectiveDpiX);
        
        // Determine quality impact message and colors
        let qualityImpactMessage = '';
        let qualityImpactColor = '#ccc';
        let dpiColor = effectiveDpiQuality.color; // DPI number uses quality-based color
        
        if (maxScale > 1) {
          // Image is enlarged - quality loss
          qualityImpactMessage = `${qualityLossX}% quality loss from resizing`;
          qualityImpactColor = '#ff6b6b'; // Red for loss
        } else if (maxScale < 1) {
          // Image is shrunk - quality gain
          qualityImpactMessage = `${Math.abs(qualityLossX)}% quality gain from resizing`;
          qualityImpactColor = '#51cf66'; // Green for gain
        } else {
          // No scaling - no change
          qualityImpactMessage = 'No quality change';
          qualityImpactColor = '#ccc';
        }
        


        // Update quality notification badge
        updateQualityNotification(effectiveDpiX);

        propertiesHTML = `
          <div style="margin-bottom: 16px;">
            <div style="font-size: 12px; color: #ccc; line-height: 1.4;">
              <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid #444;">
                <div style="font-weight: bold; color: #f4a012; margin-bottom: 4px;">DPI Quality (based on ${currentPreset.toUpperCase()}):</div>
                <div><strong>Source Image:</strong> <span style="color: ${dpiXQuality.color};">${dpiInfo.dpiX} DPI (${dpiXQuality.quality})</span></div>
                ${obj._wasAutoEnlarged && !obj._userHasResized ? `
                <div style="margin-top: 6px; padding: 6px; background: rgba(255, 193, 7, 0.1); border-left: 3px solid #ffc107; border-radius: 2px;">
                  <div style="color: #ffc107; font-size: 11px; font-weight: bold; margin-bottom: 4px;">
                    ⚠️ Your image was automatically enlarged to fit the window
                  </div>
                  <button onclick="undoAutoEnlargement('${obj._imageId}')" 
                          style="background: #ffc107; color: #222; border: none; padding: 4px 8px; border-radius: 3px; font-size: 10px; font-weight: bold; cursor: pointer;">
                    Revert to original size
                  </button>
                </div>
                ` : ''}
                <div style="margin-top: 4px; padding-top: 4px; border-top: 1px solid #333;">
                  <div style="font-weight: bold; color: #f4a012; margin-bottom: 2px; display: flex; align-items: center; gap: 6px;">
                    Current Quality:
                    ${effectiveDpiX < 72 ? '<span style="color: #ff4444; font-weight: bold; font-size: 14px;">!</span>' : ''}
                  </div>
                  <div><span style="color: ${dpiColor};">${effectiveDpiX} DPI (${effectiveDpiQuality.quality})</span></div>
                  <div><span style="color: ${qualityImpactColor};">${qualityImpactMessage}</span></div>
                </div>
              </div>
            </div>
          </div>
    ...
    // Update arrow button states based on selected layer position
    function updateLayerArrowButtonStates() {
      const moveUpBtn = document.getElementById('layer-move-up');
      const moveDownBtn = document.getElementById('layer-move-down');
      ...
      if (!activeLayerId || layers.length <= 1) {
        // No layer selected or only one layer - disable both buttons
        moveUpBtn.disabled = true;
        moveDownBtn.disabled = true;
        return;
      }
      
      // Get sorted layers (top to bottom in UI)
      const sortedLayers = [...layers].sort((a, b) => b.order - a.order);
      const selectedLayerIndex = sortedLayers.findIndex(l => l.id === activeLayerId);
      
      if (selectedLayerIndex === -1) {
        // Selected layer not found - disable both buttons
        moveUpBtn.disabled = true;
        moveDownBtn.disabled = true;
        return;
      }
      ...
      moveUpBtn.disabled = selectedLayerIndex === 0; // Can't move up if at top
      moveDownBtn.disabled = selectedLayerIndex === sortedLayers.length - 1; // Can't move down if at bottom
    }
    ...
    function handleLayerMouseDown(e) {
      ...
      if (layerItem && dragHandle && !e.target.closest('.layer-control-btn') && !e.target.closest('.layer-name')) {
    ...
    function updateAlignmentToolbar() {
      ...
      const editImageButton = ['edit-image'];
      const cropInCanvasButton = ['crop-in-canvas'];
      const editTextButton = ['edit-text'];
      ...
      if (activeObject) {
        ...
        if (activeObject.type === 'i-text' || activeObject.type === 'text') {
          ...
          // Show edit text button, hide edit image button and crop button
          editTextButton.forEach(btnId => {
            const btn = document.getElementById(btnId);
            if (btn) btn.style.display = 'flex';
          });
          editImageButton.forEach(btnId => {
            const btn = document.getElementById(btnId);
            if (btn) btn.style.display = 'none';
          });
          cropInCanvasButton.forEach(btnId => {
            const btn = document.getElementById(btnId);
            if (btn) btn.style.display = 'none';
          });
          ...
          // Show edit image button and crop button, hide edit text button
          editImageButton.forEach(btnId => {
            const btn = document.getElementById(btnId);
            if (btn) btn.style.display = 'flex';
          });
          ...
          // Hide both edit buttons for shapes
          editImageButton.forEach(btnId => {
            const btn = document.getElementById(btnId);
            if (btn) btn.style.display = 'none';
          });
        ...
        // No selection - show common buttons but hide edit buttons
        [...commonButtons, ...imageButtons, ...duplicateButton, ...layerButtons].forEach(btnId => {
          const btn = document.getElementById(btnId);
          if (btn) btn.style.display = 'flex';
        });
        // Hide both edit buttons when nothing is selected
        editImageButton.forEach(btnId => {
          const btn = document.getElementById(btnId);
          if (btn) btn.style.display = 'none';
        });
    ...
    // Alignment button event listeners
          ...
          'duplicate-object', 'edit-image', 'edit-text', 'crop-in-canvas', 'bring-forward', 'send-backward'
        ];
      ...
      activeObjects.forEach((obj, index) => {
        ...
        switch(alignmentType) {
          ...
          case 'duplicate-object':
            ...
            break;
          case 'edit-image':
            // Open crop modal for the selected image
            if (obj.type === 'image') {
              openCropModal(obj._element.src, obj);
            }
            break;
          case 'edit-text':
            // Open text editor modal for the selected text
            if (obj.type === 'i-text') {
              showTextEditorModal(obj);
            }
            break;
          case 'bring-forward':
    ...
    // --- In-Canvas Crop (simple, non-rotated images; falls back to modal otherwise) ---
    let inCanvasCrop = null;

    function getActiveFabricCanvas() {
      // Prefer desktop modal canvas if open; otherwise use mobile canvas
      if (typeof deskPadModalCanvas !== 'undefined' && deskPadModalCanvas) return deskPadModalCanvas;
      if (typeof window !== 'undefined' && window.mobileCanvas) return window.mobileCanvas;
      return null;
    }

    function positionInCanvasCropBar(barEl) {
      const isLandscape = (window.matchMedia && window.matchMedia('(orientation: landscape)').matches) || (window.innerWidth > window.innerHeight);
      const isMobile = !!(typeof window !== 'undefined' && window.mobileCanvas);

      // Desktop: center below canvas container (clamped to viewport)
      if (!isMobile) {
        const cont = document.getElementById('desk-pad-modal-canvas-container');
        if (cont) {
          const r = cont.getBoundingClientRect();
          const w = barEl.offsetWidth || 220;
          const x = Math.max(8, Math.min(window.innerWidth - w - 8, Math.round(r.left + (r.width - w) / 2)));
          const y = Math.min(window.innerHeight - 56, Math.round(r.bottom + 12));
          ...
          barEl.style.bottom = '32px';
          barEl.style.justifyContent = 'center';
        }
        return;
      }

      // Mobile landscape: top-right
      if (isLandscape) {
        barEl.style.left = 'auto';
        barEl.style.right = '16px';
        barEl.style.top = 'calc(16px + env(safe-area-inset-top))';
        barEl.style.bottom = 'auto';
        barEl.style.justifyContent = 'flex-end';
        return;
      }

      // Mobile portrait: above whichever bottom control is open; else near bottom inset
      const pad = 56; // push noticeably higher above controls
      const ids = ['mobile-transform-menu', 'mobile-bottom-row'];
      let maxH = 0;
      ...
      barEl.style.top = 'auto';
      if (maxH > 0) {
        barEl.style.bottom = Math.round(maxH + pad) + 'px';
      } else {
        barEl.style.bottom = 'calc(48px + env(safe-area-inset-bottom))';
      }
      barEl.style.justifyContent = 'center';
    }

    function enterInCanvasCropMode(imageObj) {
      const canvas = (typeof window !== 'undefined' && window.mobileCanvas) ? window.mobileCanvas : getActiveFabricCanvas();
      if (!canvas || !imageObj) return;
      // If image is rotated or flipped, use the robust modal crop instead
      if ((imageObj.angle && Math.abs(imageObj.angle % 360) > 0.01) || imageObj.flipX || imageObj.flipY) {
        try { openCropModal(imageObj._element?.src || imageObj.toDataURL?.(), imageObj); } catch(_) {}
        return;
      }
      // Cleanup any existing crop
      exitInCanvasCropMode();
      const img = imageObj;
      const imgScaledW = img.getScaledWidth();
      const imgScaledH = img.getScaledHeight();
      const center = img.getCenterPoint();

      // Create a centered crop rect at 60% of image display size
      const initialW = Math.max(50, Math.round(imgScaledW * 0.6));
      const initialH = Math.max(50, Math.round(imgScaledH * 0.6));
      const cropRect = new fabric.Rect({
        ...
        lockRotation: true,
        cornerColor: '#ffe066',
        cornerStrokeColor: '#222',
        transparentCorners: false,
      });

      canvas.add(cropRect);
      try { canvas.bringToFront(cropRect); } catch(_) {}
      canvas.setActiveObject(cropRect);
      canvas.requestRenderAll();

      // Retry placement if rect is zero due to pending layout
      requestAnimationFrame(() => {
        const bb = cropRect.getBoundingRect(true);
        if (bb.width < 2 || bb.height < 2) {
          const imgBB = img.getBoundingRect(true);
          cropRect.set({ left: imgBB.left + imgBB.width / 2, top: imgBB.top + imgBB.height / 2 });
          cropRect.setCoords();
          canvas.requestRenderAll();
        }
      });

      // Floating bar for apply/cancel with responsive placement
      const bar = document.createElement('div');
      bar.id = 'in-canvas-crop-bar';
      bar.style.cssText = 'position:fixed;display:flex;gap:12px;padding:0 16px;z-index:20010;pointer-events:auto;';
      const btnApply = document.createElement('button');
      btnApply.textContent = 'Apply Crop';
      btnApply.style.cssText = 'height:40px;min-width:108px;padding:0 12px;border-radius:10px;font-weight:700;font-size:13px;border:1px solid var(--border-color);background:var(--accent-color);color:var(--bg-primary);';
      const btnCancel = document.createElement('button');
      btnCancel.textContent = 'Cancel';
      btnCancel.style.cssText = 'height:40px;min-width:88px;padding:0 12px;border-radius:10px;font-weight:700;font-size:13px;border:1px solid var(--border-color);background:#333;color:#fff;';
      bar.appendChild(btnApply); bar.appendChild(btnCancel);
      document.body.appendChild(bar);
      positionInCanvasCropBar(bar);
      const onResize = () => positionInCanvasCropBar(bar);
      window.addEventListener('resize', onResize);
      window.addEventListener('orientationchange', onResize);

      btnApply.addEventListener('click', () => applyInCanvasCrop(img, cropRect));
      btnCancel.addEventListener('click', () => exitInCanvasCropMode());

      inCanvasCrop = { image: img, rect: cropRect, bar, onResize, canvas };
    }

    function exitInCanvasCropMode() {
      if (!inCanvasCrop) return;
      try { inCanvasCrop.canvas?.remove(inCanvasCrop.rect); } catch(_) {}
      try { inCanvasCrop.rect = null; } catch(_) {}
      try { inCanvasCrop.bar?.remove(); } catch(_) {}
      if (inCanvasCrop.onResize) {
        window.removeEventListener('resize', inCanvasCrop.onResize);
        window.removeEventListener('orientationchange', inCanvasCrop.onResize);
      }
      const c = inCanvasCrop.canvas;
      inCanvasCrop = null;
      try { c?.requestRenderAll(); } catch(_) {}
    }

    function applyInCanvasCrop(img, rect) {
      const canvas = inCanvasCrop?.canvas || getActiveFabricCanvas();
      if (!canvas || !img || !rect) return;
      // Only support non-rotated, non-flipped in this path
      if ((img.angle && Math.abs(img.angle % 360) > 0.01) || img.flipX || img.flipY) {
        exitInCanvasCropMode();
        try { openCropModal(img._element?.src || img.toDataURL?.(), img); } catch(_) {}
        return;
      }

      // Compute image display bbox (top-left in canvas coords)
      const imgScaledW = img.getScaledWidth();
      const imgScaledH = img.getScaledHeight();
      const imgOriginX = img.originX || 'left';
      const imgOriginY = img.originY || 'top';
      let imgDisplayLeft = img.left;
      let imgDisplayTop = img.top;
      if (imgOriginX === 'center') imgDisplayLeft = img.left - imgScaledW / 2; else if (imgOriginX === 'right') imgDisplayLeft = img.left - imgScaledW;
      if (imgOriginY === 'center') imgDisplayTop = img.top - imgScaledH / 2; else if (imgOriginY === 'bottom') imgDisplayTop = img.top - imgScaledH;

      // Rect displayed dimensions
      const rectDisplayW = rect.width * (rect.scaleX || 1);
      const rectDisplayH = rect.height * (rect.scaleY || 1);
      const rectOriginX = rect.originX || 'left';
      const rectOriginY = rect.originY || 'top';
      let rectDisplayLeft = rect.left;
      let rectDisplayTop = rect.top;
      if (rectOriginX === 'center') rectDisplayLeft = rect.left - rectDisplayW / 2; else if (rectOriginX === 'right') rectDisplayLeft = rect.left - rectDisplayW;
      if (rectOriginY === 'center') rectDisplayTop = rect.top - rectDisplayH / 2; else if (rectOriginY === 'bottom') rectDisplayTop = rect.top - rectDisplayH;

      // Intersection with image display area
      const sxDisplay = Math.max(0, rectDisplayLeft - imgDisplayLeft);
      const syDisplay = Math.max(0, rectDisplayTop - imgDisplayTop);
      const exDisplay = Math.min(imgScaledW, (rectDisplayLeft - imgDisplayLeft) + rectDisplayW);
      const eyDisplay = Math.min(imgScaledH, (rectDisplayTop - imgDisplayTop) + rectDisplayH);
      const cropDisplayW = Math.max(0, exDisplay - sxDisplay);
      const cropDisplayH = Math.max(0, eyDisplay - syDisplay);
      if (cropDisplayW < 2 || cropDisplayH < 2) { exitInCanvasCropMode(); return; }

      // Map to original pixels
      const srcCropX = Math.round((sxDisplay / imgScaledW) * img.width);
      const srcCropY = Math.round((syDisplay / imgScaledH) * img.height);
      const srcCropW = Math.round((cropDisplayW / imgScaledW) * img.width);
      const srcCropH = Math.round((cropDisplayH / imgScaledH) * img.height);
      if (srcCropW < 2 || srcCropH < 2) { exitInCanvasCropMode(); return; }

      // Render crop from original image element
      try {
        const tempCanvas = document.createElement('canvas');
        const tempCtx = tempCanvas.getContext('2d');
        tempCanvas.width = srcCropW;
        tempCanvas.height = srcCropH;
        if (img._element && img._element.complete) {
          tempCtx.drawImage(img._element, srcCropX, srcCropY, srcCropW, srcCropH, 0, 0, srcCropW, srcCropH);
        } else {
          exitInCanvasCropMode();
          openCropModal(img._element?.src || img.toDataURL?.(), img);
          return;
        }

        fabric.Image.fromURL(tempCanvas.toDataURL(), newImg => {
          // Preserve placement and scale relative to previous image
          ...
          newImg._originalDataUrl = img._originalDataUrl;
          newImg._originalWidth = img._originalWidth;
          newImg._originalHeight = img._originalHeight;
          if (img._proxyDataUrl) newImg._proxyDataUrl = img._proxyDataUrl;
          if (img._originalFile) newImg._originalFile = img._originalFile;
          newImg._userHasResized = true;
          newImg._imageId = img._imageId;

          updateLayerObjectReference(img, newImg);
          exitInCanvasCropMode();
        });
      } catch(err) {
    ...
    function showShapeEditorModal(shape) {
      ...
      const fillColor = shape.fill || '#ffffff';
      ...
      // Set stroke toggle based on whether stroke exists
      const hasStroke = shape.stroke && shape.strokeWidth > 0;
      document.getElementById('shape-stroke-toggle').checked = hasStroke;
      
      // Show/hide stroke controls based on toggle
    ...
    function setupShapeEditorEvents() {
      ...
      document.getElementById('shape-transparent-center').onchange = function() {
        // If transparent center is checked, auto-enable stroke
        if (this.checked) {
          document.getElementById('shape-stroke-toggle').checked = true;
          document.getElementById('stroke-controls').style.display = 'block';
        }
        updateShapePreview();
      };
      
      // Stroke toggle checkbox
      document.getElementById('shape-stroke-toggle').onchange = function() {
        const strokeControls = document.getElementById('stroke-controls');
    ...
    function applyShapeChanges() {
      ...
      const isStrokeEnabled = document.getElementById('shape-stroke-toggle').checked;
      ...
      // Determine final stroke properties based on stroke toggle
      const finalStrokeColor = isStrokeEnabled ? strokeColor : null;
      const finalStrokeWidth = isStrokeEnabled ? strokeWidth : 0;
    ...
    function updateShapePreview() {
      ...
      // Determine final stroke properties for preview
      const finalStrokeColor = isStrokeEnabled ? strokeColor : null;
      const finalStrokeWidth = isStrokeEnabled ? strokeWidth : 0;
      
      generateShapePreview(currentEditingShape, finalFillColor, finalStrokeColor, finalStrokeWidth);
    }

    // ===== PROXY MODE MANAGEMENT =====
    
    // Global variable to track current mode
    let currentMode = 'proxy'; // 'proxy' or 'quality'
    
    // Initialize proxy mode system
    function initializeProxyMode() {
      console.log('Initializing proxy mode system...');
      
      // Set up mode toggle event listeners
      const proxyBtn = document.getElementById('proxy-mode-btn');
      const qualityBtn = document.getElementById('quality-mode-btn');
      const modeInfo = document.getElementById('mode-info');
      
      if (proxyBtn && qualityBtn) {
        proxyBtn.addEventListener('click', () => switchToMode('proxy'));
        qualityBtn.addEventListener('click', () => switchToMode('quality'));
      }
      
      // Initialize in proxy mode by default
      switchToMode('proxy');
    }
    // Switch between proxy and quality modes
    function switchToMode(mode) {
      ...
      if (mode === 'proxy') {
        proxyBtn.style.background = '#f4a012';
        proxyBtn.style.color = '#000';
        qualityBtn.style.background = '#333';
        qualityBtn.style.color = '#ccc';
        qualityBtn.style.border = '1px solid #555';
        
        if (modeInfo) {
          modeInfo.innerHTML = '<strong>Performance Mode:</strong> Fast editing with proxy images, high-res export';
          modeInfo.style.borderLeftColor = '#f4a012';
        }
      } else {
        qualityBtn.style.background = '#f4a012';
        qualityBtn.style.color = '#000';
        proxyBtn.style.background = '#333';
        proxyBtn.style.color = '#ccc';
        proxyBtn.style.border = '1px solid #555';
        
        if (modeInfo) {
          modeInfo.innerHTML = '<strong>Quality Mode:</strong> High-DPI editing, may be slower';
          modeInfo.style.borderLeftColor = '#f4a012';
        }
      }
      
      // Restore download button to original state when switching to quality mode
      if (mode === 'quality') {
        const downloadBtn = document.getElementById('desk-pad-download-btn');
        if (downloadBtn && downloadBtn.originalOnclick) {
          console.log('Restoring download button to original state...');
          downloadBtn.onclick = downloadBtn.originalOnclick;
          downloadBtn.textContent = downloadBtn.originalText || 'Download';
          downloadBtn.style.background = downloadBtn.originalBackground || '';
          downloadBtn.style.color = downloadBtn.originalColor || '';
          downloadBtn.disabled = false;
          
          // Clear proxy system data
          window.readyForDownload = null;
        }
      }
      
      // Show warning if switching modes with existing content that might be affected
      if (deskPadModalCanvas && deskPadModalCanvas.getObjects().length > 0) {
        // Only show warning if there are image objects (which are affected by proxy system)
        const hasImageObjects = deskPadModalCanvas.getObjects().some(obj => 
          obj.type === 'image' && obj.hasProxyManager
        );
        
        if (hasImageObjects) {
          showModeSwitchWarning(mode);
        }
      }
    }
    
    // Show warning when switching modes with existing content
    function showModeSwitchWarning(newMode) {
      const currentModeName = currentMode === 'proxy' ? 'Performance' : 'Quality';
      const newModeName = newMode === 'proxy' ? 'Performance' : 'Quality';
      
      const warning = confirm(
        `Switching from ${currentModeName} to ${newModeName} mode with existing images may cause display issues.\n\n` +
        `Images loaded in ${currentModeName} mode may not display correctly in ${newModeName} mode.\n\n` +
        `It's recommended to clear the canvas first for best results.\n\n` +
        `Continue anyway?`
      );
      
      if (!warning) {
        // Revert the mode switch
        switchToMode(currentMode === 'proxy' ? 'quality' : 'proxy');
      }
    }
    ...
    function isQualityMode() {
      return currentMode === 'quality';
    }
    
    // Override the image loading function to use proxy system when in proxy mode
    function addImageToModalCanvasWithMode(imageDataUrl, imageName = null, index = 0) {
      if (isProxyMode()) {
        // Use proxy system
        console.log('Using proxy system for image loading...');
        
        // Convert data URL to File object for proxy system
        fetch(imageDataUrl)
          .then(res => res.blob())
          .then(blob => {
            const file = new File([blob], imageName || 'image.jpg', { type: 'image/jpeg' });
            
            if (window.addImageToCanvasWithProxy && deskPadModalCanvas) {
              window.addImageToCanvasWithProxy(file, deskPadModalCanvas, index);
            } else {
              console.error('Proxy system not available, falling back to quality mode');
              addImageToModalCanvasInternal(imageDataUrl, imageName, index);
            }
          })
          .catch(error => {
            console.error('Error converting to file for proxy system:', error);
            addImageToModalCanvasInternal(imageDataUrl, imageName, index);
          });
      } else {
        // Use original quality mode
        console.log('Using quality mode for image loading...');
        addImageToModalCanvasInternal(imageDataUrl, imageName, index);
      }
    }
    
    // Override the download function to use proxy system when in proxy mode
    function downloadCanvasWithMode() {
      if (isProxyMode()) {
        // Use proxy system for export
        console.log('Using proxy system for export...');
        
        // Check if proxy system has already prepared a download
        if (window.readyForDownload && window.readyForDownload.canvas) {
          console.log('Proxy system already has export ready, download will be handled by button onclick...');
          // Don't trigger download here - let the button's onclick handle it
          return; // Exit early to prevent double-triggering
        } else if (window.enhancedDownloadCanvas && deskPadModalCanvas) {
          // Prepare the export first
          window.enhancedDownloadCanvas(deskPadModalCanvas);
        } else {
          console.error('Proxy system not available, falling back to quality mode');
          downloadCanvas();
        }
        ...
        // Use original quality mode
        console.log('Using quality mode for export...');
        downloadCanvas();
      }
    }